...
# Constants for clone generation
CLONE_COUNT = 8  # Number of clones to generate
MIN_DISTANCE = 5.0  # Minimum distance between clones
MAX_DISTANCE = 25.0  # Maximum distance between clones
ROAD_WIDTH = 10.0  # Approximate road width
CLONE_ID_OFFSET = 100000  # Offset for clone IDs to avoid conflicts

# Random name generation
FIRST_NAMES = ["Alex", "Sam", "Chris", "Pat", "Jordan", "Taylor", "Morgan", "Casey", 
               "Robin", "Jamie", "Quinn", "Riley", "Avery", "Parker", "Charlie"]
LAST_NAMES = ["Smith", "Johnson", "Williams", "Brown", "Jones", "Garcia", "Miller",
              "Davis", "Rodriguez", "Martinez", "Hernandez", "Lopez", "Wilson"]

clone_states = {}  # 存储克隆玩家状态
...
def generate_formation_position(center_x, center_z, index, total):
    """生成更自然的集团阵型位置"""
    # 使用更自然的队形分布算法
    # 基于蜂窝���六边形网格，但添加随机偏移
    row = index // 3
    ...
    # 基础位置（类似于不规则网格）
    base_x = (col - 1) * MIN_DISTANCE
    base_z = row * MIN_DISTANCE * 0.866  # sqrt(3)/2 for hexagonal packing
    
    # 添加随机偏移，但保持在道路范围内
    random_offset_x = random.uniform(-1, 1) * MIN_DISTANCE * 0.3
    random_offset_z = random.uniform(-1, 1) * MIN_DISTANCE * 0.3
    
    # 确保不会偏离太远
    final_x = center_x + base_x + random_offset_x
    final_z = center_z + base_z + random_offset_z
    
    # 限制在道路宽度范围内
    final_x = max(min(final_x, center_x + ROAD_WIDTH/2), center_x - ROAD_WIDTH/2)
    
    return final_x, final_z

def generate_clone_states(player_state, player_id):
    """生成克隆玩家的状态"""
    if player_id not in clone_states:
        clone_states[player_id] = []
    
    clones = []
    for i in range(CLONE_COUNT):
        clone = udp_node_msgs_pb2.PlayerState()
        clone.CopyFrom(player_state)
        
        # 生成随机名字
        first_name, last_name = generate_random_name()
        clone.id = player_id + CLONE_ID_OFFSET + i
        
        # 生成队形位置
        clone_x, clone_z = generate_formation_position(player_state.x, player_state.z, i, CLONE_COUNT)
        clone.x = clone_x
        clone.z = clone_z
        
        # 根据位置调整速度和功率
        # 后排骑手速度略快（追赶效应）
        position_factor = 1.0 + (clone.z - player_state.z) * 0.01
        clone.speed = int(player_state.speed * position_factor * (0.95 + random.random() * 0.1))
        clone.power = int(player_state.power * position_factor * (0.95 + random.random() * 0.1))
        
        # 保持相同的高度和方向
        clone.y_altitude = player_state.y_altitude
        clone.heading = player_state.heading
        
        clones.append(clone)
    
    clone_states[player_id] = clones
    return clones
...
class UDPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        if state.roadTime:
            ...
            # 生成克隆玩家状态
            if state.roadTime:  # 只在玩家实际骑行时生成克隆
                clone_states = generate_clone_states(state, player_id)
                # 将克隆玩家添加到在线玩家列表
                for clone in clone_states:
                    online[clone.id] = clone
